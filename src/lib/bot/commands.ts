...
export const MAIN_KEYBOARD = new Keyboard()
  .text("📊 Дашборд").text("📖 Допомога").text("📋 Правила").row()
  .text("📅 Звіт тижня").text("📅 Звіт дня").text("🔄 Скинути")
  .resized()
  .persistent();
...
*Навчи мене правилу і я запам'ятаю назавжди:*
...
export async function handleSchedule(ctx: BotContext): Promise<void> {
  ...
  await ctx.reply(
    ...
    `Час: ${schedule.time}\n\n` +
    `Просто напиши мені що змінити, наприклад:\n` +
    `_"Вмикай тижневий звіт о 10:00"_\n` +
    `_"Вимкни щоденний звіт"_\n` +
    `_"Хочу місячний звіт о 9:00"_\n` +
    `_"Змінити час звітів на 20:00"_`,
    { parse_mode: "Markdown" }
  );
}
