...
function metricColor(key: string) {
  return METRIC_COLORS[key] ?? { bg: 'bg-[var(--tgui--secondary_bg_color)]', text: 'text-[var(--tgui--hint_color)]' };
}
...
function DrillDownDrawer({ title, entries, onClose, onUpdate, accessToken }: {
  ...
  return (
    <>
      ...
      <div
        className="fixed bottom-0 left-0 right-0 z-50 rounded-t-2xl bg-[var(--tgui--bg_color)] shadow-2xl"
      ...
      >
        ...
        <div className="flex-shrink-0 pt-3 pb-2 flex justify-center">
          <div className="h-1 w-10 rounded-full bg-[var(--tgui--secondary_bg_color)]" />
        </div>

        {/* Header */}
        <div className="flex-shrink-0 flex items-center justify-between px-5 pb-3">
          <h3 className="font-semibold">{title}</h3>
          <button onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--tgui--secondary_bg_color)] text-[var(--tgui--hint_color)]">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-2">
          {entries.length === 0 && (
            <p className="py-8 text-center text-sm text-[var(--tgui--hint_color)]">Немає записів</p>
          )}
          <div className="flex flex-col gap-2">
            {entries.map((e) => (
              <Card
                key={e.id}
                className="cursor-pointer p-3 transition-colors hover:bg-[var(--tgui--secondary_bg_color)]/30"
              ...
              >
                <div className="mb-1.5 flex items-center justify-between gap-2">
                  ...
                  <time className="text-[10px] text-[var(--tgui--hint_color)]">
      ...
      <div className="relative w-full rounded-t-2xl bg-[var(--tgui--bg_color)] px-4 pt-4 pb-8 shadow-2xl">
          ...
          {(['today','week','month'] as DateRange[]).map(r => (
            <button key={r} onClick={() => apply(r)}
              className={cn('rounded-xl border py-3 text-sm font-medium transition-colors',
                filter.range === r ? 'border-[var(--tgui--button_color)] bg-[var(--tgui--button_color)] text-[var(--tgui--button_text_color)]' : 'border-[var(--tgui--hint_color)]/20 bg-[var(--tgui--secondary_bg_color)]/30 text-[var(--tgui--text_color)]')}>
              {RANGE_LABELS[r]}
            </button>
          ))}
        </div>
        <div className="mb-3 flex items-center gap-2">
          <input type="date" value={fromStr} onChange={e => setFromStr(e.target.value)}
            className="flex-1 rounded-xl border border-[var(--tgui--hint_color)] bg-[var(--tgui--bg_color)] px-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-[var(--tgui--link_color)]" />
          <span className="text-[var(--tgui--hint_color)]">–</span>
  ...
  if (goals.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <p className="text-sm text-[var(--tgui--hint_color)]">Цілей ще немає</p>
        <p className="mt-1 text-xs text-[var(--tgui--hint_color)]">Скажи боту: &ldquo;Хочу пробігти 100км цього місяця&rdquo;</p>
      </div>
    );
  }
          ...
          <Card key={g.key} className="p-4">
            <div className="mb-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-[var(--tgui--hint_color)]">Ціль: {g.target} {g.unit}{g.period ? ` / ${g.period}` : ''}</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold">{actual?.value ?? 0}</p>
                <p className="text-xs text-[var(--tgui--hint_color)]">з {g.target} {g.unit}</p>
              </div>
            </div>
            <div className="h-2 w-full overflow-hidden rounded-full bg-[var(--tgui--secondary_bg_color)]">
              <div className={cn('h-full rounded-full transition-all', pct >= 100 ? 'bg-green-400' : 'bg-[var(--tgui--link_color)]/70')} style={{ width: `${pct}%` }} />
            </div>
            <p className="mt-1 text-right text-[10px] text-[var(--tgui--hint_color)]">{pct}%</p>
          </Card>
        );
      })}
    </div>
...
function MetricCard({ metric, sourceEntries, onEntryClick, goal }: {
  ...
  return (
    <Card
      className="flex cursor-pointer flex-col gap-1 p-4 transition-colors hover:bg-[var(--tgui--secondary_bg_color)]/30 active:bg-[var(--tgui--secondary_bg_color)]/50"
    ...
    >
      ...
      <div className="flex items-baseline gap-1">
        ...
        {goal && <span className="text-sm text-[var(--tgui--hint_color)]">/ {goal.target}</span>}
        <span className="text-sm text-[var(--tgui--hint_color)]">{metric.unit}</span>
      </div>
      ...
      {pct !== null ? (
        <div className="mt-1">
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-[var(--tgui--secondary_bg_color)]">
            <div
              className={cn('h-full rounded-full transition-all', pct >= 100 ? 'bg-green-400' : 'bg-[var(--tgui--button_color)]/60')}
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
      ) : (
        <p className="text-[10px] text-[var(--tgui--hint_color)]">{aggLabel}</p>
      )}
    </Card>
  );
}
  ...
  return (
    <Card className="col-span-2 p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <p className="text-xs font-medium uppercase tracking-wide text-[var(--tgui--hint_color)]">Енергетичний баланс</p>
      </div>
      <div className="flex items-center justify-between gap-4">
        <div className="text-center">
          <p className="text-lg font-bold text-orange-500">{intake.toLocaleString()}</p>
          <p className="text-[10px] text-[var(--tgui--hint_color)]">з&apos;їдено ккал</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-bold text-blue-500">{burned.toLocaleString()}</p>
          <p className="text-[10px] text-[var(--tgui--hint_color)]">спалено ккал</p>
        </div>
        <div className="text-center">
          <p className={cn('text-lg font-bold', isDeficit ? 'text-green-600' : 'text-red-500')}>
            {isDeficit ? '' : '+'}{net.toLocaleString()}
          </p>
          <p className="text-[10px] text-[var(--tgui--hint_color)]">{isDeficit ? 'дефіцит' : 'профіцит'}</p>
        </div>
      </div>
      {/* Visual bar */}
      <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-[var(--tgui--secondary_bg_color)]">
        <div
    ...
    <div>
      <button onClick={() => setOpen(o => !o)} className="mb-3 flex w-full items-center justify-between py-0.5">
        <div className="flex items-center gap-2">
          ...
          {count !== undefined && <span className="rounded-full bg-[var(--tgui--secondary_bg_color)] px-1.5 py-0.5 text-[10px] text-[var(--tgui--hint_color)]">{count}</span>}
        </div>
        <ChevronDown size={15} className={cn('text-[var(--tgui--hint_color)] transition-transform duration-200', !open && '-rotate-90')} />
      </button>
      {open && children}
    </div>
  );
...
function SpendBar({ label, pct, amount, currency }: { label: string; pct: number; amount: number; currency: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className="w-20 shrink-0 truncate text-xs capitalize text-[var(--tgui--hint_color)]">{label}</span>
      <div className="flex-1 overflow-hidden rounded-full bg-[var(--tgui--secondary_bg_color)]" style={{ height: 6 }}>
...
export default function DashboardPage() {
  ...
  const moodColor = moodAvg === null ? 'bg-[var(--tgui--secondary_bg_color)] text-[var(--tgui--hint_color)]' : moodAvg > 0.5 ? 'bg-green-100 text-green-700' : moodAvg < -0.5 ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700';
    ...
    <div className="flex flex-col gap-4 px-4 pt-5 pb-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-full bg-[var(--tgui--secondary_bg_color)] px-3 py-1.5 text-xs font-medium text-[var(--tgui--hint_color)]"
        >
          <Calendar size={13} />
        ...
        <TabsContent value="actual">
          ...
          {status === 'error' && (
            <div className="py-12 text-center">
              <p className="text-sm text-[var(--tgui--hint_color)]">Не вдалося завантажити дані</p>
              <Button size="sm" className="mt-3" onClick={() => fetchEntries(filter.from, filter.to)}>Повторити</Button>
            </div>
          )}
          {status === 'ready' && entries.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <p className="text-sm text-[var(--tgui--hint_color)]">Немає записів за цей період</p>
            </div>
          )}
